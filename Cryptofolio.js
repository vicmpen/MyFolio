...
const baseURL =
  "http://api.coinmarketcap.com/v1/ticker=1500&convert=EUR&limit=200";
const baseURLv2 =
  "http://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?start=1&limit=600&convert=EUR";
...
process.argv.forEach(function(val, index, array) {
  if (val == "history") {
    fs.readFile(path.normalize(historyFile), "utf8", (error, data) => {
...
const printBalance = coinInfo => {
  ...
  if (userCoins && userCoins.length > 0) {
    ...
    userCoins.forEach(coin => {
      ...
      sum += euroPrice;
      console.log(
        ` ** ${coin.symbol} @ ${coin.quote[currencyKey].price} (${
          coin.quote[currencyKey].percent_change_1h
        } since an hour ago) ==> ${euroPrice.toFixed(2)} euro
          `
      );
      newLine();
    });
    ...
    console.log(
      "currentTotal EURO ====>",
      parseInt(sum),
      `${upOrDown} ${difference} euro since ${utils.msToTime(
        Date.parse(new Date()) - Date.parse(lastDate)
      )} hours ago`
    );
    console.log("\n====================================");
    console.log(
      "currentTotal ETH ====>",
      parseFloat(sum / ethPrice).toFixed(2)
    );
...
const readInfoFile = path => {
  if (!fs.existsSync(path)) {
    ...
    if (coins.length !== coinAmounts.length) {
      console.log("====================================");
      console.log(
        "Missmatching info, Make sure the number of coins and amounts is the same, exiting"
      );
      console.log("====================================");
      process.exit();
    }
...
program
  ...
  .option(
    "-f, --file [file]",
    "Provide a text file with two rows. The first has comma separated CMC coin Symbols (e.g. eth) and the second the corresponding amount you own",
    ""
  )
  .parse(process.argv);
...
getAllCoins().then(response => {
  ...
  ethPrice = parseFloat(
    response.data.data.filter(currency => currency.symbol === "ETH")[0].quote[
      currencyKey
    ].price
  ).toFixed(2);
  if (program.file) {
    try {
      ...
      response.data.data.forEach(currency => {
        if (currency.symbol) availableCurrencies[currency.symbol] = currency;
      });
